...
def load_trained_model(model_path="models/cattle_classifier.pth", num_classes=10, device="cpu", backbone="mobilenet_v3_large"):
    ...
    # Flexible path candidate lookup
    ...
    if not os.path.exists(resolved_path):
        candidates = [
            ...
            os.path.join(base_dir, "models", "cattle_classifier.pth"),
            os.path.join(project_root, "models", "cattle_classifier.pth")
        ]
        for c in candidates:
            if os.path.exists(c):
                resolved_path = c
    ...
    if os.path.exists(resolved_path):
        ...
        print(f"Loaded trained model weights from '{resolved_path}' successfully.")
    else:
