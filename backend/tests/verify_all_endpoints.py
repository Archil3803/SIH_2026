...
def main():
    ...
    # 2. Test CSS & JS Assets (both direct and legacy paths)
    test_endpoint("Direct CSS Stylesheet", f"{BASE_URL}/css/style.css")
    test_endpoint("Direct JS Script", f"{BASE_URL}/js/app.js")
    test_endpoint("Legacy CSS Stylesheet", f"{BASE_URL}/static/css/style.css")
    test_endpoint("Legacy JS Script", f"{BASE_URL}/static/js/app.js")
    ...
    # 5. Test Breeds Catalog API
    status, breeds_bytes = test_endpoint("All Breeds API", f"{BASE_URL}/api/breeds")
    breeds_data = json.loads(breeds_bytes)
    assert breeds_data.get("count") >= 10, f"Expected at least 10 breeds, got {breeds_data.get('count')}"

    # 6. Test Specific Breed API
    ...
    # Resolve actual sample file location for client upload simulation
    sample_full_path = sample_path
    if not os.path.exists(sample_full_path):
        candidates = [
            PROJECT_ROOT / "dataset" / sample_path,
            PROJECT_ROOT / sample_path,
            BACKEND_DIR / "dataset" / sample_path
        ]
        for c in candidates:
            if c.exists():
                sample_full_path = str(c)
                break

    # 10. Test Predict API via Multipart File Upload
    ...
    # 11. Test Predict API on Non-Bovine Image (Must return error alert without probabilities)
    ...
    # 12. Test Multi-Bovine Detection API
    comp_img = Image.new("RGB", (620, 300), color=(240, 240, 240))
    img1 = Image.open(sample_full_path).convert("RGB").resize((300, 300))
